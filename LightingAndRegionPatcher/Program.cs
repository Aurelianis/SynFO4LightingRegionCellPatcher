...
namespace LightingAndRegionPatcher
{
    ...
    {
        //Name of all ESPs that are patched
        ...
        public static void RunPatch(IPatcherState<IFallout4Mod, IFallout4ModGetter> state)
        {
            ...
            var clarity = state.LoadOrder.GetIfEnabled(ClarityEspName);
            var uil = state.LoadOrder.GetIfEnabled(UILEspName);
            var regions = state.LoadOrder.GetIfEnabled(RegionEspName);
            var jsrsregions = state.LoadOrder.GetIfEnabled(JSRSRegionEspName);
            var bosstory = state.LoadOrder.GetIfEnabled(BoSStoryEspName);

            //If Ultra Interior Lighting is Enabled, forward cell lighting records
            if (uil.Mod != null)
            {
                Console.WriteLine($"Processing {uil.ModKey}");
